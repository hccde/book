...
异常
异常的条件码也会被压入栈中
如果控制用户从用户态转到内核态 所有这些项目都被压入内核栈中 而不是压入用户栈
异常发生在内核态

异常包括 中断  故障 和 陷阱（系统调用）
中断是异步（突发）的 陷阱和故障 终止是同步的
...
进程在内核执行系统调用的时候 比如说进行了读文件 这通常需要很久的时间 处理器不会停在那里等 它会切换到另一个进程运行 比如切换到进程B
它切换到进程B的时候是先在B的内核态执行 然后再切换到B的用户态执行 这个是读文件的中断完成了，根据操作系统的调度 有可能B走完了时间片 然后再执行A
也有可能执行A

进程的生命周期：
1运行：进程要么在cpu上运行 要么等待运行并且最终会被cpu调度
2停止：进程的执行被挂起，且不会被调度。当收到sigstop sigtstp sigttin或者sigttou信号的时候进程就停止 并且停止到它收到一个sigcont信号。在这个时刻，进程再次开始运行。（信号是一种软件终端的形式）
3终止 进程永远的停止了，进程会因为三种原因终止 1 收到一个信号 该信号的默认行为是终止进程。 2从主程序返回 3 调用exit函数

对于fork函数来说 它调用一次返回 两次  父进程中返回一次 子进程中也返回一次
当有多次fork的时候 fork出来的第一个子进程还会继续执行fork 所以fork的子进程数不一定是等于fork函数条数，注意画出fork的进程树

当进程停止的时候 并不会立即被回收 而是直到他的父进程被回收 当父进程被回收的时候 子进程还在运行的话 子进程会被挂到进程1 init进程 下面

程序和进程的区别 程序是一堆代码和数据 进程是执行过程中程序的一个具体实例
一个进程最多只有一个同类型的信号

因为并发运行和信号量带来一些处理上的问题 函数的运行带来了不可预测性
1 处理程序要尽可能简单
2处理程序中只调用异步安全的函数，所谓的异步安全 要么它是可重入没有副作用的函数 要么它是阻塞的不能被处理程序中断的
3 保存和回复errno.许多linux异步信号安全的函数都会在出错的时候返回设置errno.在处理程序中调用这样的函数可能会干扰主程序中其他依赖errno的部分，
解决办法是用一个局部变量保存它。
4用volatile声明全局变量。考虑一个处理程序和main函数共享全局变量。 告诉编译器不要缓存这个变量 强制每次从内存读 而不是放在寄存器或者缓存中，
因为这个变量是共享的处理程序会更新它 main会读取它 而在编译器看来 这个变量似乎没有变 就会被缓存 但是它是变化了的 是处理程序改变了它。用volatile来修饰变量的时候 要注意阻塞信号 防止被中断。
5 阻塞所有信号，保存对共享全局数据的访问。
6用sig_atomic_t声明标志。c提供一种整数数据类型 sig_atomic_t 对它的读写会保证是原子的（不可中断的）	
